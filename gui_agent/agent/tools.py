...
be used by a planner, a LangChain adapter, or manual end-to-end demos.
"""
...
def create_langchain_tools(state: AgentToolState) -> list[Any]:
    """Create LangChain StructuredTool adapters when langchain-core is installed."""
    ...
    return [
        StructuredTool.from_function(lambda delay=0.0, monitor_index=None: capture_screen_tool(state, delay=delay, monitor_index=monitor_index), name="capture_screen", description="Capture the current desktop screen."),
        StructuredTool.from_function(lambda lang="ch", use_gpu=True, delay=0.0, monitor_index=None: ocr_screen_tool(state, lang=lang, use_gpu=use_gpu, delay=delay, monitor_index=monitor_index), name="ocr_screen", description="Capture the screen and run OCR."),
        StructuredTool.from_function(lambda keyword, min_confidence=0.0, fuzzy_threshold=None, return_all=False, regex=False: find_text_tool(state, keyword, min_confidence=min_confidence, fuzzy_threshold=fuzzy_threshold, return_all=return_all, regex=regex), name="find_text", description="Find text from the latest OCR result."),
        StructuredTool.from_function(lambda keyword, monitor_index=None, min_confidence=0.0, fuzzy_threshold=None, regex=False: click_text_tool(state, keyword, monitor_index=monitor_index, min_confidence=min_confidence, fuzzy_threshold=fuzzy_threshold, regex=regex), name="click_text", description="Click the latest OCR element matching the given text."),
